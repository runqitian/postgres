...
new_objtree_for_role_id(Oid roleoid)
{
	...
	if (roleoid != ACL_ID_PUBLIC)
	{
		...
		if (!HeapTupleIsValid(roltup))
			elog(ERROR, "role with OID %u does not exist", roleoid);
...
deparse_AlterFunction(Oid objectId, Node *parsetree)
{
	...
	if (isprocedure)
		alterFunc = new_objtree("ALTER PROCEDURE");
	else
		alterFunc = new_objtree(node->objtype == OBJECT_ROUTINE ?
	...
	foreach(cell, node->actions)
	{
		...
		{
			tmpobj = new_objtree(strVal(defel->arg));
		}
		else if (strcmp(defel->defname, "strict") == 0)
		{
			tmpobj = new_objtree(boolVal(defel->arg) ?
...
deparse_GrantStmt(CollectedCommand *cmd)
{
	...
	if (istmt->grantor_uid)
	{
		...
		if (!HeapTupleIsValid(roltup))
			elog(ERROR, "role with OID %u does not exist", istmt->grantor_uid);
	...
	foreach(cell, cmd->d.opfam.operators)
	{
		...
		if (!stmt->isDrop)
		{
			...
			else
			{
				...
				tmpobj2 = new_objtree("FOR ORDER BY");
...
deparse_CompositeTypeStmt(Oid objectId, Node *parsetree)
{
	...
	composite = new_objtree("CREATE TYPE");
...
deparse_AlterEnumStmt(Oid objectId, Node *parsetree)
{
	...
	alterEnum = new_objtree("ALTER TYPE");
...
deparse_AlterExtensionStmt(Oid objectId, Node *parsetree)
{
	...
	stmt = new_objtree_VA("ALTER EXTENSION %{identity}I UPDATE", 1,
...
deparse_AlterDefaultPrivilegesStmt(CollectedCommand *cmd)
{
	...
	alterStmt = new_objtree("ALTER DEFAULT PRIVILEGES");
	...
	append_string_object(grant, "ON %{target}s", objtype);
	...
	append_object_object(alterStmt, "%{grant}s", grant);
	...
	foreach(cell, cmd->d.alterTable.subcmds)
	{
		...
		switch (subcmd->subtype)
		{
			...
			case AT_ForceRowSecurity:
				tmpobj = new_objtree("FORCE ROW LEVEL SECURITY");
				subcmds = lappend(subcmds, new_object_object(tmpobj));
				break;

			case AT_NoForceRowSecurity:
				tmpobj = new_objtree("NO FORCE ROW LEVEL SECURITY");
				subcmds = lappend(subcmds, new_object_object(tmpobj));
				break;
			...
			case AT_AddIdentity:
				{
					...
					if (OidIsValid(seq_relid))
					{
						...
						append_object_object(tmpobj, "ADD %{identity_column}s", seqdef);
					}
	...
	if (!composite)
	{
		...
		switch (attrForm->attstorage)
		{
			...
			case 'm':
				...
				break;
			default:
				elog(ERROR, "invalid storage specifier %c", attrForm->attstorage);
		}

		append_string_object(column, "STORAGE %{colstorage}s", str);
	}
...
deparse_CreateDomain(Oid objectId, Node *parsetree)
{
	 ...
	 * %{collation}s
	 */
	createDomain = new_objtree_VA("CREATE DOMAIN %{identity}D AS %{type}T %{not_null}s", 3,
								  "identity", ObjTypeObject,
								  new_objtree_for_qualname_id(TypeRelationId,
															  objectId),
								  "type", ObjTypeObject,
	...
	if (defval)
		append_string_object(createDomain, "DEFAULT %{default}s", defval);
...
deparse_CreateFunction(Oid objectId, Node *parsetree)
{
	 ...
	 * %{symbol}L
	 */
	createFunc = new_objtree_VA("CREATE %{or_replace}s", 1,
...
deparse_CreateOpClassStmt(CollectedCommand *cmd)
{
	 ...
	 */
	stmt = new_objtree_VA("CREATE OPERATOR CLASS %{identity}D %{default}s FOR TYPE %{type}T USING %{amname}I", 4,
						  "identity", ObjTypeObject,
	...
	foreach(cell, cmd->d.createopc.operators)
	{
		...
		else
		{
			...
			tmpobj2 = new_objtree("FOR ORDER BY %{opfamily}D");
 ...
 *
 * Given a operator family OID and the parse tree that created it, return the
 * JSON blob representing the creation command.
 */
static ObjTree *
 ...
 *
 * Given a policy OID and the parse tree that created it, return the
 * JSON blob representing the creation command.
 */
static ObjTree *
deparse_CreatePolicyStmt(Oid objectId, Node *parsetree)
{
	...
	add_policy_clauses(policy, objectId, node->roles, node->qual != NULL,
 ...
 * the JSON blob representing the alter command.
 */
static ObjTree *
...
deparse_CreateSchemaStmt(Oid objectId, Node *parsetree)
{
	 ...
	 */
	createSchema = new_objtree_VA("CREATE SCHEMA %{if_not_exists}s %{name}s", 2,
						...
						ObjectAddress constraintAddr)
{
	...
	switch (node->subtype)
	{
		case 'T':
			...
			if (node->def == NULL)
				alterDom = new_objtree_VA("ALTER DOMAIN %{identity}D DROP DEFAULT", 2,
										  "type", ObjTypeString, "drop default",
										  "identity", ObjTypeObject,
										  new_objtree_for_qualname(domForm->typnamespace,
																   NameStr(domForm->typname)));
			else
				alterDom = new_objtree_VA("ALTER DOMAIN %{identity}D SET DEFAULT %{default}s", 3,
										  "type", ObjTypeString, "set default",
										  "identity", ObjTypeObject,
										  new_objtree_for_qualname(domForm->typnamespace,
																   NameStr(domForm->typname)),
		...
		case 'N':
			/* DROP NOT NULL */
			alterDom = new_objtree_VA("ALTER DOMAIN %{identity}D DROP NOT NULL", 2,
									  "type", ObjTypeString, "drop not null",
									  "identity", ObjTypeObject,
									  new_objtree_for_qualname(domForm->typnamespace,
															   NameStr(domForm->typname)));
		...
		case 'O':
			/* SET NOT NULL */
			alterDom = new_objtree_VA("ALTER DOMAIN %{identity}D SET NOT NULL", 2,
									  "type", ObjTypeString, "set not null",
									  "identity", ObjTypeObject,
									  new_objtree_for_qualname(domForm->typnamespace,
															   NameStr(domForm->typname)));
		...
		case 'C':
			 ...
			 * domains
			 */
			alterDom = new_objtree_VA("ALTER DOMAIN %{identity}D ADD CONSTRAINT %{constraint_name}s %{definition}s", 4,
									  "type", ObjTypeString, "add constraint",
									  "identity", ObjTypeObject,
									  new_objtree_for_qualname(domForm->typnamespace,
															   NameStr(domForm->typname)),
		...
		case 'V':
			 ...
			 * requires its name.
			 */
			alterDom = new_objtree_VA("ALTER DOMAIN %{identity}D VALIDATE CONSTRAINT %{constraint_name}s", 3,
									  "type", ObjTypeString, "validate constraint",
									  "identity", ObjTypeObject,
									  new_objtree_for_qualname(domForm->typnamespace,
															   NameStr(domForm->typname)),
...
deparse_CreateStatisticsStmt(Oid objectId, Node *parsetree)
{
	...
	createStat = new_objtree_VA("CREATE STATISTICS %{if_not_exists}s", 1,
								"if_not_exists", ObjTypeString,
								node->if_not_exists ? "IF NOT EXISTS" : "");
	...
	append_object_object(createStat, "FROM %{stat_table_identity}D",
...
deparse_CreateLangStmt(Oid objectId, Node *parsetree)
{
	...
	createLang = new_objtree("CREATE");

	append_string_object(createLang, "%{or_replace}s",
						 node->replace ? "OR REPLACE" : "");
	append_string_object(createLang, "%{trusted}s",
						 langForm->lanpltrusted ? "TRUSTED" : "");
	append_string_object(createLang, "LANGUAGE %{identity}s", node->plname);

	if (node->plhandler != NIL)
	{
		/* Add the HANDLER clause */
		append_object_object(createLang, "HANDLER %{handler}D",
							 new_objtree_for_qualname_id(ProcedureRelationId,
		...
		if (OidIsValid(langForm->laninline))
		{
			...
			append_bool_object(tmp, "present", false);
		append_object_object(createLang, "%{inline}s", tmp);
		...
		if (OidIsValid(langForm->lanvalidator))
		{
			...
			append_bool_object(tmp, "present", false);
		append_object_object(createLang, "%{validator}s", tmp);
	}
...
deparse_CreateStmt(Oid objectId, Node *parsetree)
{
	 ...
	 */
	createStmt = new_objtree_VA("CREATE %{persistence}s TABLE %{if_not_exists}s %{identity}D", 3,
							 ...
							 ObjectAddress fromCollid)
{
	...
	stmt = new_objtree_VA("CREATE COLLATION %{identity}D", 1,
						  "identity", ObjTypeObject,
						  new_objtree_for_qualname(colForm->collnamespace,
												   NameStr(colForm->collname)));
							...
							ObjectAddress copied)
{
	 ...
	 */
	stmt = new_objtree_VA("CREATE TEXT SEARCH CONFIGURATION %{identity}D", 1,
						  "identity", ObjTypeObject,
...
deparse_DefineStmt_TSParser(Oid objectId, DefineStmt *define)
{
	 ...
	 */
	stmt = new_objtree_VA("CREATE TEXT SEARCH PARSER %{identity}D", 1,
						  "identity", ObjTypeObject,
						  new_objtree_for_qualname(tspForm->prsnamespace,
												   NameStr(tspForm->prsname)));
...
deparse_DefineStmt_TSDictionary(Oid objectId, DefineStmt *define)
{
	 ...
	 */
	stmt = new_objtree_VA("CREATE TEXT SEARCH DICTIONARY %{identity}D", 1,
						  "identity", ObjTypeObject,
...
deparse_DefineStmt_TSTemplate(Oid objectId, DefineStmt *define)
{
	 ...
	 */
	stmt = new_objtree_VA("CREATE TEXT SEARCH TEMPLATE %{identity}D", 1,
						  "identity", ObjTypeObject,
...
deparse_AlterTSDictionaryStmt(Oid objectId, Node *parsetree)
{
	 ...
	 */
	alterTSD = new_objtree_VA("ALTER TEXT SEARCH DICTIONARY %{identity}D", 1,
							  "identity", ObjTypeObject,
							  new_objtree_for_qualname(tsdForm->dictnamespace,
													   NameStr(tsdForm->dictname)));
...
deparse_ViewStmt(Oid objectId, Node *parsetree)
{
	 ...
	 */
	viewStmt = new_objtree_VA("CREATE %{or_replace}s %{persistence}s VIEW %{identity}D AS %{query}s", 4,
							  "or_replace", ObjTypeString,
							  node->replace ? "OR REPLACE" : "",
							  "persistence", ObjTypeString,
							  get_persistence_str(relation->rd_rel->relpersistence),
...
deparse_CreateTableAsStmt_vanilla(Oid objectId, Node *parsetree)
{
	...
	if (node->into->colNames)
	{
		...
		tmp = new_objtree_VA("", 1, "present", ObjTypeBool, false);
...
deparse_CreateTrigStmt(Oid objectId, Node *parsetree)
{
	 ...
	 */
	trigger = new_objtree_VA("CREATE %{constraint}s TRIGGER %{name}I", 2,
	...
	append_string_object(trigger, "%{time}s", trigtiming);
...
deparse_CreateUserMappingStmt(Oid objectId, Node *parsetree)
{
	...
	createStmt = new_objtree_VA("CREATE USER MAPPING FOR %{role}R SERVER %{server}I", 2,
								"role", ObjTypeObject,
...
deparse_AlterUserMappingStmt(Oid objectId, Node *parsetree)
{
	...
	alterStmt = new_objtree_VA("ALTER USER MAPPING FOR %{role}R SERVER %{server}I", 2,
							   "role", ObjTypeObject,
							   new_objtree_for_role_id(form->umuser),
							   "server", ObjTypeString, server->servername);
...
deparse_AlterStatsStmt(Oid objectId, Node *parsetree)
{
	...
	alterStat = new_objtree_VA("ALTER STATISTICS %{identity}D SET STATISTICS %{target}n", 2,
							   "identity", ObjTypeObject,
							   new_objtree_for_qualname(statform->stxnamespace,
														NameStr(statform->stxname)),
...
deparse_RefreshMatViewStmt(Oid objectId, Node *parsetree)
{
	 ...
	 */
	refreshStmt = new_objtree_VA("REFRESH MATERIALIZED VIEW %{concurrently}s %{identity}D", 2,
...
deparse_DefElem(DefElem *elem, bool is_reset)
{
	...
	set = new_objtree_VA("%{label}s", 1, "label", ObjTypeObject, optname);
					 ...
					 DropBehavior behavior)
{
	...
	tmpobj = new_objtree_VA("CASCADE", 1,
							"present", ObjTypeBool, behavior == DROP_CASCADE);
	append_object_object(stmt, "%{cascade}s", tmpobj);
...
deparse_FunctionSet(VariableSetKind kind, char *name, char *value)
{
	...
	else if (kind == VAR_SET_VALUE)
	{
		...
		obj = new_objtree_VA("RESET %{set_name}I", 1,
							 "set_name", ObjTypeString, name);
...
deparse_IndexStmt(Oid objectId, Node *parsetree)
{
	 ...
	 * %{where_clause}s
	 */
	indexStmt = new_objtree_VA("CREATE %{unique}s INDEX %{concurrently}s %{if_not_exists}s %{name}I ON %{table}D USING %{index_am}s (%{definition}s)", 7,
...
deparse_RenameStmt(ObjectAddress address, Node *parsetree)
{
	...
	switch (node->renameType)
	{
		...
		case OBJECT_MATVIEW:
			...
			renameStmt = new_objtree_VA(fmtstr, 3,
		...
		case OBJECT_POLICY:
			{
				...
				renameStmt = new_objtree_VA("ALTER POLICY %{if_exists}s %{policyname}I ON %{identity}D RENAME TO %{newname}I", 4,
											"if_exists", ObjTypeString,
											node->missing_ok ? "IF EXISTS" : "",
		...
		case OBJECT_COLUMN:
			...
			{
				fmtstr = psprintf("ALTER %s", stringify_objtype(node->relationType));
		...
		case OBJECT_OPCLASS:
			{
				...
				fmtstr = psprintf("ALTER %s %%{identity}D USING %%{index_method}s RENAME TO %%{newname}I",
								  stringify_objtype(node->renameType));
...
deparse_AlterDependStmt(Oid objectId, Node *parsetree)
{
	...
	if (node->objectType == OBJECT_INDEX)
	{
		...
		stmt = new_objtree_VA("ALTER INDEX %{identity}D %{no}s DEPENDS ON EXTENSION %{newname}I", 3,
...
deparse_Seq_As(DefElem *elem)
{
	...
	asoption = new_objtree_VA("AS %{identity}D",
							  1,
							  "identity", ObjTypeObject,
	...
	return new_object_object(asoption);
}
...
deparse_RuleStmt(Oid objectId, Node *parsetree)
{
	...
	tmp = new_objtree("");
	if (qual)
		append_string_object(tmp, "WHERE %{clause}s", qual);
	else
	{
	...
	ruleStmt = new_objtree_VA("CREATE RULE %{or_replace}s %{identity}I AS ON %{event}s TO %{table}D %{where_clause}s DO %{instead}s %{actions:, }s", 7,
							  "or_replace", ObjTypeString,
							  node->replace ? "OR REPLACE" : "",
...
deparse_CreateTransformStmt(Oid objectId, Node *parsetree)
{
	 ...
	 * %{signature_tof}s )
	 */
	createTransform = new_objtree_VA("CREATE %{or_replace}s TRANSFORM FOR %{typename}D LANGUAGE %{language}I", 3,
									 "or_replace", ObjTypeString,
									 node->replace ? "OR REPLACE" : "",
	...
	if (OidIsValid(trfForm->trffromsql))
	{
		...
		append_object_object(createTransform, "(FROM SQL WITH FUNCTION %{signature}s", signature);
		ReleaseSysCache(procTup);
	}
	if (OidIsValid(trfForm->trftosql))
	{
		...
		append_object_object(createTransform, "TO SQL WITH FUNCTION %{signature_tof}s",
							 signature);
		ReleaseSysCache(procTup);
	}
...
append_literal_or_null(ObjTree *parent, char *elemname, char *value)
{
	...
	top = new_objtree("");
	part = new_objtree_VA("NULL", 1, "present", ObjTypeBool, !value);
	append_object_object(top, "%{null}s", part);

	part = new_objtree_VA("", 1, "present", ObjTypeBool, value != NULL);
...
deparse_CommentOnConstraintSmt(Oid objectId, Node *parsetree)
{
	 ...
	 */
	comment = new_objtree_VA("COMMENT ON CONSTRAINT %{identity}s ON %{domain}s %{parentobj}s", 3,
	...
	append_literal_or_null(comment, "IS %{comment}s", node->comment);
 ...
 *
 * Given the object address and the parse tree that created it, return the JSON
 * blob representing the comment command.
 */
static ObjTree *
deparse_CommentStmt(ObjectAddress address, Node *parsetree)
{
	 ...
	 */
	comment = new_objtree_VA("COMMENT ON %{objtype}s", 1,
							 "objtype", ObjTypeString,
							 (char *) stringify_objtype(node->objtype));
	...
	append_string_object(comment, "%{identity}s", identity);
...
deparse_CreateAmStmt(Oid objectId, Node *parsetree)
{
	 ...
	 */
	CreateAm = new_objtree_VA("CREATE ACCESS METHOD %{identity}I TYPE %{am_type}s HANDLER %{handler}D", 3,
							  "identity", ObjTypeString,
							  NameStr(amForm->amname),
