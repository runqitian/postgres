...
/*
 * EventTriggerCollectCreatePublication
 *		Save data about a CREATE PUBLICATION command being executed
 */
void
EventTriggerCollectCreatePublication(CreatePublicationStmt *stmt,
									 ...
									 List *schemas)
{
	...
	command = palloc0(sizeof(CollectedCommand));
	command->type = SCT_CreatePublication;
	command->in_extension = creating_extension;
	ObjectAddressSet(command->d.createpub.address,
					 PublicationRelationId, pubid);
	command->parsetree = (Node *) copyObject(stmt);

	foreach(lc, relations)
	{
		PublicationTable *pub_rel = (PublicationTable *) lfirst(lc);

		publication_rel *addrel = (publication_rel *) palloc(sizeof(publication_rel));

		Relation	rel = table_openrv(pub_rel->relation, AccessShareLock);

		addrel->relid = RelationGetRelid(rel);
		addrel->whereClause = copyObject(pub_rel->whereClause);
		addrel->columnList = list_copy_deep(pub_rel->columns);
		command->d.createpub.rels = lappend(command->d.createpub.rels, addrel);
		table_close(rel, AccessShareLock);
	}

	command->d.createpub.schemas = list_copy(schemas);
...
/*
 * EventTriggerCollectAlterPublication
 *		Save data about a ALTER PUBLICATION command being executed
 */
void
EventTriggerCollectAlterPublication(AlterPublicationStmt *stmt, Oid pubid,
									List *relations, List *schemas)
{
	MemoryContext oldcxt;
	CollectedCommand *command;
	ListCell   *lc;
	...
	command = palloc0(sizeof(CollectedCommand));
	command->type = SCT_AlterPublication;
	command->in_extension = creating_extension;
	ObjectAddressSet(command->d.alterpub.address,
					 PublicationRelationId, pubid);
	command->parsetree = (Node *) copyObject(stmt);
	...
	foreach(lc, relations)
	{
		...
		addrel->whereClause = copyObject(pub_rel->whereClause);
		addrel->columnList = list_copy_deep(pub_rel->columns);
		command->d.alterpub.rels = lappend(command->d.alterpub.rels, addrel);
		table_close(rel, AccessShareLock);
	}

	command->d.alterpub.schemas = list_copy(schemas);
			...
			case SCT_AlterPublication:
				{
					...
					else if (cmd->type == SCT_CreatePublication)
						addr = cmd->d.createpub.address;
					else if (cmd->type == SCT_AlterPublication)
						addr = cmd->d.alterpub.address;
